...
# Data to CSV URI
def datatocsv(data):
    # Making a file-like string and writing list to it
    csv_sio = StringIO()
    csv.writer(csv_sio).writerows(data)
    # Do this awfulness turning it into a data URI string and returning
    csv_uri = 'data:text/csv;base64,' + base64.b64encode(str.encode(csv_sio.getvalue())).decode()
    return csv_uri
    ...
    for scrobble in data[1:]:
        ...
        if datecompare(currentdate, type) != datecompare(scrbldate, type):
            ...
            while datecompare(currentdate + relativedelta(**{type: 1}), type) < datecompare(scrbldate, type):
                ...
                print('Filling ' + currentdate.strftime(strftype(type)) + ' AS EMPTY!!!')
                df = pd.concat([df, pd.DataFrame(dict, index=[currentdate.strftime(strftype(type))])])
    ...
    while True:
        ...
        for scrobble in reversed(scrobbles):
            ...
            if ts and int(scrobble['date']['uts']) <= ts:
                print('Scrobble is old, skipping')
                continue
...
def update(file, api):
    ...
    while True:
        ...
        for scrobble in reversed(scrobbles):
            ...
            if scrbldate == ts:
                print('Found the last one! Starting fetch from page ' + str(parameters['page']))
        ...
        # Switch to a new page
        print(f"UPDATE: {parameters['page']} out of {pages}")
        parameters['page'] += 1
